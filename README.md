# 운정고등학교 입시설명회 신청 좌석 조회 시스템입니다.

학생 이름, 학생 전화번호, 보호자 전화번호 뒤 4자리가 기존 신청 정보와 모두 일치할 때 배정 좌석을 보여줍니다. 조회 결과는 캐시하지 않으며 Redis로 반복 조회를 제한합니다. 기존 신청 데이터와 관리자 기능은 유지하되, 기본 설정에서는 신규 티켓팅 입장을 차단합니다.

운영 환경에서 티켓팅을 다시 열어야 할 때만 `TICKETING_ENABLED=1`을 명시적으로 설정하세요. 조회 제한은 `LOOKUP_RATE_LIMIT`(기본 10회)과 `LOOKUP_RATE_WINDOW_SECONDS`(기본 600초)로 조정할 수 있습니다.
...
설정합니다. 서버는 개인정보가 없는 조회 폼만 기본 300초간 CDN에 저장하고, 조회 결과·기존 세션·대기열·신청·
